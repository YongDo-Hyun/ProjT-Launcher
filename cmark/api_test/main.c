...
static const cmark_node_type node_types[] = { CMARK_NODE_DOCUMENT,	CMARK_NODE_BLOCK_QUOTE, CMARK_NODE_LIST,
...
static void accessors(test_batch_runner* runner)
{
	...
	static const char expected_html[] = "<h3>Header</h3>\n"
										"<ol start=\"3\">\n"
										"<li>\n"
										"<p>Item 1</p>\n"
										"</li>\n"
...
void hierarchy(test_batch_runner* runner)
{
	...
	int all_inlines = (1 << CMARK_NODE_TEXT) | (1 << CMARK_NODE_SOFTBREAK) | (1 << CMARK_NODE_LINEBREAK)
					| (1 << CMARK_NODE_CODE) | (1 << CMARK_NODE_HTML_INLINE) | (1 << CMARK_NODE_EMPH)
...
static void sub_document(test_batch_runner* runner)
{
...

	{
		...
		cmark_parser* parser =
			cmark_parser_new_with_mem_into_root(CMARK_OPT_DEFAULT, cmark_get_default_mem_allocator(), item);
...
{
	{
		static const char markdown[] = "*first*\n"
									   "second\n";
	...
	{
		static const char markdown[] = "` It is one backtick\n"
