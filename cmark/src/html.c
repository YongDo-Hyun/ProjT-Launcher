...
static int S_render_node(cmark_node* node, cmark_event_type ev_type, struct render_state* state, int options)
{
	...
	if (state->plain != NULL)
	{
		switch (node->type)
		{
			...
			case CMARK_NODE_HTML_INLINE: escape_html(html, node->data, node->len); break;

			case CMARK_NODE_LINEBREAK:
			case CMARK_NODE_SOFTBREAK: cmark_strbuf_putc(html, ' '); break;
	...
	switch (node->type)
	{
		...
		case CMARK_NODE_TEXT: escape_html(html, node->data, node->len); break;
