...
static int S_render_node(cmark_renderer* renderer, cmark_node* node, cmark_event_type ev_type, int options)
{
	...
	else
	{
		if (node->type == CMARK_NODE_LIST)
		{
			renderer->in_tight_list_item =
				node->parent && node->parent->type == CMARK_NODE_ITEM && node->parent->parent->as.list.tight;
		}
	}
	...
	switch (node->type)
	{
		...
		case CMARK_NODE_HTML_INLINE: OUT(cmark_node_get_literal(node), false, LITERAL); break;
