...
static inline bool can_contain(cmark_node_type parent_type, cmark_node_type child_type)
{
	return (parent_type == CMARK_NODE_DOCUMENT || parent_type == CMARK_NODE_BLOCK_QUOTE
			|| parent_type == CMARK_NODE_ITEM || (parent_type == CMARK_NODE_LIST && child_type == CMARK_NODE_ITEM));
}
...
static bufsize_t parse_list_marker(cmark_mem* mem,
								   cmark_chunk* input,
								   bufsize_t pos,
...
static void S_parser_feed(cmark_parser* parser, const unsigned char* buffer, size_t len, bool eof)
{
	...
	if (parser->line_number == 0 && parser->column == 0 && len >= 3 && *buffer == 0xEF && *(buffer + 1) == 0xBB
		&& *(buffer + 2) == 0xBF)
	{
		buffer += 3;
	}
								  ...
								  cmark_chunk* input)
{
	...
	const bool last_line_blank =
		(parser->blank && ctype != CMARK_NODE_BLOCK_QUOTE && ctype != CMARK_NODE_HEADING
		 && ctype != CMARK_NODE_THEMATIC_BREAK && !(ctype == CMARK_NODE_CODE_BLOCK && container->as.code.fenced)
		...
		else if (S_type(container) == CMARK_NODE_HTML_BLOCK)
		{
			...
			switch (container->as.html_block_type)
			{
				...
				case 5:
					...
					break;
				default: matches_end_condition = 0; break;
			}
