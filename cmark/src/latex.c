...
static inline void outc(cmark_renderer* renderer, cmark_escaping escape, int32_t c, unsigned char nextc)
{
	...
	switch (c)
	{
		...
		case 8211: // endash
			...
			break;
		default: cmark_render_code_point(renderer, c);
	}
}
	...
	switch (node->type)
	{
		...
		case CMARK_NODE_HEADING:
			if (entering)
			{
				...
				{
					case 1: LIT("\\section"); break;
					case 2: LIT("\\subsection"); break;
					case 3: LIT("\\subsubsection"); break;
					case 4: LIT("\\paragraph"); break;
					case 5: LIT("\\subparagraph"); break;
				}
				LIT("{");
			}
			else
		...
		case CMARK_NODE_HTML_BLOCK: break;
		...
		case CMARK_NODE_TEXT: OUT(cmark_node_get_literal(node), allow_wrap, NORMAL); break;
		...
		case CMARK_NODE_HTML_INLINE: break;
