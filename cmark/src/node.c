...
static bool S_can_contain(cmark_node* node, cmark_node* child)
{
	...
	switch (node->type)
	{
		...
		case CMARK_NODE_ITEM: return cmark_node_is_block(child) && child->type != CMARK_NODE_ITEM;

		case CMARK_NODE_LIST: return child->type == CMARK_NODE_ITEM;

		case CMARK_NODE_CUSTOM_BLOCK: return true;
		...
		case CMARK_NODE_CUSTOM_INLINE: return cmark_node_is_inline(child);
...
cmark_node* cmark_node_new_with_mem(cmark_node_type type, cmark_mem* mem)
{
	...
	switch (node->type)
	{
		case CMARK_NODE_HEADING: node->as.heading.level = 1; break;
...
static void S_free_nodes(cmark_node* e)
{
	...
	while (e != NULL)
	{
		switch (e->type)
		{
			...
			case CMARK_NODE_HTML_BLOCK: mem->free(e->data); break;
			case CMARK_NODE_LINK:
			case CMARK_NODE_IMAGE:
...
const char* cmark_node_get_literal(cmark_node* node)
{
	...
	switch (node->type)
	{
		...
		case CMARK_NODE_HTML_INLINE:
		case CMARK_NODE_CODE:
		case CMARK_NODE_CODE_BLOCK: return node->data ? (char*)node->data : "";
...
int cmark_node_set_literal(cmark_node* node, const char* content)
{
	...
	switch (node->type)
	{
		...
		case CMARK_NODE_HTML_INLINE:
		case CMARK_NODE_CODE:
		case CMARK_NODE_CODE_BLOCK: node->len = cmark_set_cstr(node->mem, &node->data, content); return 1;
...
int cmark_node_get_heading_level(cmark_node* node)
{
	...
	switch (node->type)
	{
		case CMARK_NODE_HEADING: return node->as.heading.level;
...
int cmark_node_set_heading_level(cmark_node* node, int level)
{
	...
	switch (node->type)
	{
		case CMARK_NODE_HEADING: node->as.heading.level = level; return 1;
...
const char* cmark_node_get_url(cmark_node* node)
{
	...
	{
		case CMARK_NODE_LINK:
		case CMARK_NODE_IMAGE: return node->as.link.url ? (char*)node->as.link.url : "";
		default: break;
	}
...
int cmark_node_set_url(cmark_node* node, const char* url)
{
	...
	{
		case CMARK_NODE_LINK:
		case CMARK_NODE_IMAGE: cmark_set_cstr(node->mem, &node->as.link.url, url); return 1;
		default: break;
	}
...
const char* cmark_node_get_title(cmark_node* node)
{
	...
	{
		case CMARK_NODE_LINK:
		case CMARK_NODE_IMAGE: return node->as.link.title ? (char*)node->as.link.title : "";
		default: break;
	}
...
int cmark_node_set_title(cmark_node* node, const char* title)
{
	...
	{
		case CMARK_NODE_LINK:
		case CMARK_NODE_IMAGE: cmark_set_cstr(node->mem, &node->as.link.title, title); return 1;
		default: break;
	}
...
const char* cmark_node_get_on_enter(cmark_node* node)
{
	...
	{
		case CMARK_NODE_CUSTOM_INLINE:
		case CMARK_NODE_CUSTOM_BLOCK: return node->as.custom.on_enter ? (char*)node->as.custom.on_enter : "";
		default: break;
	}
...
int cmark_node_set_on_enter(cmark_node* node, const char* on_enter)
{
	...
	{
		case CMARK_NODE_CUSTOM_INLINE:
		case CMARK_NODE_CUSTOM_BLOCK: cmark_set_cstr(node->mem, &node->as.custom.on_enter, on_enter); return 1;
		default: break;
	}
...
const char* cmark_node_get_on_exit(cmark_node* node)
{
	...
	{
		case CMARK_NODE_CUSTOM_INLINE:
		case CMARK_NODE_CUSTOM_BLOCK: return node->as.custom.on_exit ? (char*)node->as.custom.on_exit : "";
		default: break;
	}
...
int cmark_node_set_on_exit(cmark_node* node, const char* on_exit)
{
	...
	{
		case CMARK_NODE_CUSTOM_INLINE:
		case CMARK_NODE_CUSTOM_BLOCK: cmark_set_cstr(node->mem, &node->as.custom.on_exit, on_exit); return 1;
		default: break;
	}
