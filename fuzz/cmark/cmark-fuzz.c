...
int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
	...
	if (size >= sizeof(fuzz_config))
	{
		...
		options &= (CMARK_OPT_SOURCEPOS | CMARK_OPT_HARDBREAKS | CMARK_OPT_UNSAFE | CMARK_OPT_NOBREAKS
		...
		{
			case 0: doc = cmark_parse_document(markdown, markdown_size, options); break;
			...
			case 3: free(cmark_markdown_to_html(markdown, markdown_size, options)); break;
		}
