...
class read_test : public CxxTest::TestSuite
{
  public:
	...
	void verify_bigtest_structure(const tag_compound& comp)
	{
		...
		TS_ASSERT((comp.at("nested compound test")
				   == tag_compound{ { "egg", tag_compound{ { "value", 0.5f }, { "name", "Eggbert" } } },
	...
	void test_read_misc()
	{
		...
		TS_ASSERT(*pair.second
				  == tag_string("Even though unprovided for by NBT, the library should also handle "
