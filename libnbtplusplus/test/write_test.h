...
class read_test : public CxxTest::TestSuite
{
  public:
	...
	void test_write_payload_big()
	{
		...
		TS_ASSERT_EQUALS(os.str(), (std::string{ '\x7F', '\x7F', '\xFF', '\x7F', '\xFF', '\xFF', '\xFF', '\x7F',
												 '\xFF', '\xFF', '\xFF', '\xFF', '\xFF', '\xFF', '\xFF',

												 '\xAB', '\xCD', '\xEF', '\x01', '\xAB', '\xCD', '\xEF', '\x01',
												 '\x02', '\x03', '\x04', '\x05' }));
		...
		TS_ASSERT_EQUALS(os.str(),
						 (std::string{ 0x00,
									   0x06, // string length in Big Endian
									   'b',
									   'a',
									   'r',
									   'b',
