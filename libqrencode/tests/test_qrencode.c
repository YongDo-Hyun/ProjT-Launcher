...
static void test_qrraw_new(void)
{
	...
	for (i = 0; i < raw->b1; i++)
	{
		assert_equal(raw->rsblock[i].dataLength,
					 19,
					 "QRraw.rsblock[%d].dataLength was not as expected. (%d)\n",
					 i,
	...
	{
		assert_equal(raw->rsblock[i].eccLength,
					 24,
					 "QRraw.rsblock[%d].eccLength was not as expected. (%d)\n",
					 i,
...
static void test_iterate2()
{
	...
	QRRawCode* raw;
	int err					= 0;
	unsigned char correct[] = { 0x10, 0x11, 0xec, 0xec, 0x20, 0xec, 0x11, 0x11, 0x0c, 0x11, 0xec, 0xec, 0x56, 0xec,
								0x11, 0x11, 0x61, 0x11, 0xec, 0xec, 0x80, 0xec, 0x11, 0x11, 0xec, 0x11, 0xec, 0xec,
								0x11, 0xec, 0x11, 0x11, 0xec, 0x11, 0xec, 0xec, 0x11, 0xec, 0x11, 0x11, 0xec, 0x11,
								0xec, 0xec, 0x11, 0x11, 0x5c, 0xde, 0x68, 0x68, 0x4d, 0xb3, 0xdb, 0xdb, 0xd5, 0x14,
								0xe1, 0xe1, 0x5b, 0x2a, 0x1f, 0x1f, 0x49, 0xc4, 0x78, 0x78, 0xf7, 0xe0, 0x5b, 0x5b,
								0xc3, 0xa7, 0xc1, 0xc1, 0x5d, 0x9a, 0xea, 0xea, 0x48, 0xad, 0x9d, 0x9d, 0x58, 0xb3,
								0x3f, 0x3f, 0x10, 0xdb, 0xbf, 0xbf, 0xeb, 0xec, 0x05, 0x05, 0x98, 0x35, 0x83, 0x83,
								0xa9, 0x95, 0xa6, 0xa6, 0xea, 0x7b, 0x8d, 0x8d, 0x04, 0x3c, 0x08, 0x08, 0x64, 0xce,
								0x3e, 0x3e, 0x4d, 0x9b, 0x30, 0x30, 0x4e, 0x65, 0xd6, 0xd6, 0xe4, 0x53, 0x2c, 0x2c,
...
static void test_filler(void)
{
	...
	{
		length = QRspec_getDataLength(i, QR_ECLEVEL_L) * 8 + QRspec_getECCLength(i, QR_ECLEVEL_L) * 8
...
static void test_fillerMQR(void)
{
	...
	for (i = 1; i <= MQRSPEC_VERSION_MAX; i++)
	{
		...
		else
		{
			...
			assert_equal(frame[e],
						 (unsigned char)((length - 1) & 127) | 0x80,
...
static void test_encodeLongData(void)
{
	...
	for (i = QR_MODE_NUM; i <= QR_MODE_KANJI; i++)
	{
		...
		for (l = QR_ECLEVEL_L; l <= QR_ECLEVEL_H; l++)
		{
			...
			assert_nonnull(qrcode,
						   "(QRcode_encodeInput) failed to encode %d-byte %s in level %d.\n",
						   maxlength[i][l],
						   modeStr[i],
						   l);
			if (qrcode != NULL)
			...
			if (ret == 0)
			{
				qrcode = QRcode_encodeInput(stream);
				assert_null(qrcode,
							"(QRcode_encodeInput) incorrectly succeeded to encode %d-byte %s in level %d.\n",
							len,
							modeStr[i],
							l);
				if (qrcode != NULL)
...
static void test_01234567(void)
{
	...
	unsigned char correct[] = {
		,
...
static void test_decodeSimple(void)
{
	...
	if (qrdata != NULL)
	{
		assert_equal(strlen(str),
					 qrdata->size,
					 "Lengths of input/output mismatched: %d, expected %d.\n",
					 qrdata->size,
					 (int)strlen(str));
		assert_zero(strncmp(str, (char*)(qrdata->data), qrdata->size),
					"Decoded data %s is different from the original %s\n",
					qrdata->data,
					str);
	}
	if (qrdata != NULL)
...
static void test_oddBitCalcMQR(void)
{
	...
	TestString tests[] = { { "46194", 1, QR_ECLEVEL_L, QR_MODE_8, 1 },
...
static void test_mqrencode(void)
{
	...
	qrcode.data = frame;
	qrdata		= QRcode_decodeMQR(&qrcode);
	assert_equal(qrdata->version,
				 3,
				 "Format info decoder returns wrong version number: %d (%d expected)\n",
				 qrdata->version,
				 3);
...
static void test_apiversion(void)
{
	...
	assert_equal(major_version,
				 MAJOR_VERSION,
				 "Major version number mismatched: %d (%d expected)\n",
				 major_version,
				 MAJOR_VERSION);
	assert_equal(minor_version,
				 MINOR_VERSION,
				 "Minor version number mismatched: %d (%d expected)\n",
				 minor_version,
				 MINOR_VERSION);
	assert_equal(micro_version,
				 MICRO_VERSION,
				 "Micro version number mismatched: %d (%d expected)\n",
				 micro_version,
				 MICRO_VERSION);
