...
static void test_struct_insertStructuredAppendHeaders(void)
{
	...
	while (p != NULL)
	{
		...
		assert_equal(p->input->head->data[0],
					 10,
					 "size of the structured-header is wrong: #%d, %d should be %d\n",
					 i,
					 p->input->head->data[0],
					 10);
		assert_equal(p->input->head->data[1],
					 i,
					 "index of the structured-header is wrong: #%d, %d should be %d\n",
					 i,
					 p->input->head->data[1],
					 i);
...
static void checkEstimatedVersion(int ver, int mode)
{
	...
	assert_equal(estimatedVersion,
				 ver,
				 "Estimated version %d is not equal to the expected version %d for %d %s sequence.\n",
				 estimatedVersion,
				 ver,
				 maxCharacterLengths[ver - 1][mode],
				 modeStr[mode]);
	QRinput_free(input);
	...
	assert_equal(estimatedVersion,
				 ...
				 estimatedVersion,
				 ver,
				 maxCharacterLengths[ver - 1][mode] + 1,
				 modeStr[mode]);
	QRinput_free(input);
}
