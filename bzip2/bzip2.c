...
errhandler:
	...
	switch (bzerr)
	{
		case BZ_CONFIG_ERROR: configError(); break;
		case BZ_MEM_ERROR: outOfMemory(); break;
		case BZ_IO_ERROR:
		errhandler_io:
			ioError();
			break;
		default: panic("compress:unexpected error");
	}
...
errhandler:
	...
	switch (bzerr)
	{
		...
		errhandler_io:
			ioError();
			break;
		case BZ_DATA_ERROR: crcError();
		case BZ_MEM_ERROR: outOfMemory();
		case BZ_UNEXPECTED_EOF: compressedStreamEOF();
		...
		default: panic("decompress:unexpected error");
	}
...
errhandler:
	...
	switch (bzerr)
	{
		...
		errhandler_io:
			ioError();
			break;
		case BZ_DATA_ERROR: fprintf(stderr, "data integrity (CRC) error in data\n"); return False;
		case BZ_MEM_ERROR: outOfMemory();
		case BZ_UNEXPECTED_EOF: fprintf(stderr, "file ends unexpectedly\n"); return False;
		case BZ_DATA_ERROR_MAGIC:
			if (zStream != stdin)
				fclose(zStream);
			if (streamNo == 1)
		...
		default: panic("test:unexpected error");
	}
...
static void compress(Char* name)
{
	...
	switch (srcMode)
	{
		...
		default: panic("compress: bad srcMode"); break;
	}
...
zzz:
	...
	switch (srcMode)
	{
		...
		default: panic("uncompress: bad srcMode"); break;
	}
...
static void testf(Char* name)
{
	...
	switch (srcMode)
	{
		...
		default: panic("testf: bad srcMode"); break;
	}
