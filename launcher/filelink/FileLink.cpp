				...
				switch (socketError)
				{
					...
					case QLocalSocket::ConnectionRefusedError:
						qDebug() << ("The connection was refused by the peer. "
									 ...
									 "is correct.");
						break;
					case QLocalSocket::PeerClosedError: qDebug() << ("The connection was closed by the peer. "); break;
					default: qDebug() << "The following error occurred: " << socket.errorString();
				}
			});
...
void FileLinkApp::runLink()
{
	...
	for (auto link : m_links_to_make)
	{
		...
		else if (fs::is_directory(StringUtils::toStdString(src_path)))
		{
			...
			fs::create_directory_symlink(StringUtils::toStdString(src_path),
										 StringUtils::toStdString(dst_path),
										 os_err);
		}
		else
		{
