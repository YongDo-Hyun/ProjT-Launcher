...
class CustomOAuthOobReplyHandler : public QOAuthOobReplyHandler
{
  ...
  public:
	...
	{
		return BuildConfig.LAUNCHER_APP_BINARY_NAME + "://oauth/microsoft";
	}
};
...
MSAStep::MSAStep(AccountData* data, bool silent) : AuthStep(data), m_silent(silent)
{
	...
	if (QCoreApplication::applicationFilePath().startsWith("/tmp/.mount_") || APPLICATION->isPortable()
	...
	connect(&m_oauth2,
			...
			this,
			[this, silent](const QAbstractOAuth2::Error err)
			{
...
void MSAStep::perform()
{
	if (m_silent)
	{
		...
		{
			emit finished(AccountTaskState::STATE_DISABLED,
						  tr("Microsoft user authentication failed - refresh token is empty."));
			return;
		}
