...
DeviceAuthorizationResponse parseDeviceAuthorizationResponse(const QByteArray& data)
{
	...
	auto obj = doc.object();
	return {
		Json::ensureString(obj, "device_code"),		  Json::ensureString(obj, "user_code"),
		Json::ensureString(obj, "verification_uri"),  Json::ensureInteger(obj, "expires_in"),
		Json::ensureInteger(obj, "interval"),		  Json::ensureString(obj, "error"),
	...
	if (!rsp.error.isEmpty() || !rsp.error_description.isEmpty())
	{
		...
		emit finished(AccountTaskState::STATE_FAILED_HARD,
					  tr("Device authorization failed: %1")
						  .arg(rsp.error_description.isEmpty() ? rsp.error : rsp.error_description));
		return;
	}
...
void MSADeviceCodeStep::authenticationFinished()
{
	...
	if (!rsp.error.isEmpty() || !rsp.error_description.isEmpty())
	{
		...
		emit finished(
			AccountTaskState::STATE_FAILED_HARD,
