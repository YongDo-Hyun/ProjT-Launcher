...
void XboxAuthorizationStep::onRequestDone()
{
	...
	if (m_request->error() != QNetworkReply::NoError)
	{
			...
			if (!processSTSError())
			{
				emit finished(AccountTaskState::STATE_FAILED_SOFT,
							  tr("Failed to get authorization for %1 services. Error %2.")
								  .arg(m_authorizationKind, m_request->error()));
			}
			else
			{
				emit finished(
		...
		else
		{
			emit finished(AccountTaskState::STATE_OFFLINE,
						  tr("Failed to get authorization for %1 services: %2")
							  .arg(m_authorizationKind, m_request->errorString()));
		}
		return;
	}
	...
	{
		emit finished(AccountTaskState::STATE_FAILED_SOFT,
					  tr("Server has changed %1 authorization user hash in the reply. Something is wrong.")
...
bool XboxAuthorizationStep::processSTSError()
{
	if (m_request->error() == QNetworkReply::AuthenticationRequiredError)
	{
		...
		switch (errorCode)
		{
			case 2148916233:
			{
				emit finished(
					...
					tr("This Microsoft account does not have an XBox Live profile. Buy the game on %1 first.")
						.arg(
							"<a "
							"href=\"https://www.minecraft.net/en-us/store/minecraft-java-edition\">minecraft.net</a>"));
				return true;
			}
			case 2148916235:
			{
			...
			{
				emit finished(AccountTaskState::STATE_FAILED_SOFT,
							  tr("This Microsoft account requires proof of age to play. Please login to %1 to provide "
								 "proof of age.")
			...
			{
				emit finished(AccountTaskState::STATE_FAILED_SOFT,
							  tr("This Microsoft account is currently restricted and your guardian has not given you "
								 "permission to play "
			...
			{
				emit finished(AccountTaskState::STATE_FAILED_SOFT,
							  tr("This Microsoft account has not accepted Xbox's Terms of Service. Please login and "
								 "accept them."));
				return true;
			}
			default:
